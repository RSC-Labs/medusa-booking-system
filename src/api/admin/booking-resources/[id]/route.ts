...
type PostAdminBookingResourceType = {
  booking_resource: Omit<BookingResourceType, "booking_resource_availability_rules" | "booking_resource_pricing_configs" | "booking_resource_allocations">,
...
export async function GET (
  ...
  const bookingResource = await bookingModuleService.retrieveBookingResource(req.params.id, {
    relations: ["booking_resource_availability_rules", "booking_resource_pricing_configs", "booking_resource_allocations"]
  })
  ...
  res.json({
    ...
    booking_resource_availability_rules: bookingResource.booking_resource_availability_rules,
    booking_resource_pricing_configs: transformToBookingPricingApi(products[0].variants, bookingResource.booking_resource_pricing_configs),
  })
}
...
export async function POST (
  ...
  const bookingResourceOld = await bookingModuleService.retrieveBookingResource(req.params.id)
  ...
  const bookingResourceNew = await bookingModuleService.retrieveBookingResource(req.params.id)
