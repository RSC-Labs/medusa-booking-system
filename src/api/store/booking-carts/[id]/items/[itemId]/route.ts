...
export async function DELETE (
  ...
  const { booking_resource_allocation } = await bookingModuleService.retrieveBookingCartItem(req.params["itemId"])

  const booking_cart_item = await bookingModuleService.retrieveBookingCartItem(req.params["itemId"])
  ...
  await bookingModuleService.deleteBookingResourceAllocations(booking_resource_allocation.id)
